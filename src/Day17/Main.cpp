...
// Very slow but tuples / arrays aren't hashable in the stdlib
#include <map>

long run2(std::map<std::array<long, 4>, bool> board) {
...
long run1(std::map<std::array<long, 4>, bool> board) {
  std::map<std::array<long, 4>, bool> update;
  ...
  for (const auto &[v, c] : board) {
    ...
    for (long i = -1; i <= 1; ++i) {
      for (long j = -1; j <= 1; ++j) {
        for (long k = -1; k <= 1; ++k) {
          auto p = std::array<long, 4>({x + i, y + j, z + k, 0});
          if (board.find(p) == board.end())
            update[p] = false;
          else
            update[p] = board[p];
